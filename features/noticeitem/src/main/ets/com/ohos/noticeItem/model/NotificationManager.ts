...
export default class NotificationManager {
  ...
  static requestTemplate(tag, templateName, templatePath) {
    ...
    NotificationManager.request(tag, requestParam, (err, data) => {
      ...
      Log.showDebug(TAG, `request finished templateData: ${templateName} data: ${JSON.stringify(data.componentTemplate)}`)
      if (data !== null && data !== undefined) {
        Log.showDebug(TAG, `request finished data.componentTemplate.source:${JSON.stringify(data.componentTemplate.source)}`)
        let templates = JSON.parse(data.componentTemplate.source);
        Log.showDebug(TAG, `request templates: ${JSON.stringify(templates)}`)
