...
export class ViewModel {
  ...
  onNotificationCancel(hashCode: string) {
    Log.showDebug(TAG, `onNotificationCancel hashCode: ${JSON.stringify(hashCode)}`);
  ...
  updateNotification() {
    Log.showDebug(TAG, `updateNotification list: ${JSON.stringify(this.mNotificationList)}`);
    ...
    if (itemData.notificationFlags?.soundEnabled != Constants.NOTIFICATION_TYPE_CLOSE) {
      if (itemData.ruleData.isAllowSound) {
        try {
          ...
          Log.showDebug(TAG, `sound notificationItem id：${itemData.id} alert error: ${e.toString()}`);
        }
      }
    }
