  ...
  getTrustedDeviceDeviceName(deviceId) {
    ...
    if (deviceArr && deviceArr.length > 0) {
      for (let item of deviceArr) {
        Log.showDebug(TAG, `getTrustedDeviceDeviceName deviceArr item:${JSON.stringify(item)}`);
